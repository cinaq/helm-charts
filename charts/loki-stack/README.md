...
```bash
helm upgrade --install loki cinaq/loki-stack
```

### Deploy in a custom namespace

```bash
helm upgrade --install loki --namespace=loki-stack cinaq/loki-stack
```

### Deploy with custom config

```bash
helm upgrade --install loki cinaq/loki-stack --set "key1=val1,key2=val2,..."
```
